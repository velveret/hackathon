...
#                parents[current].objId if parents[current] else None)
            ...
            if parents[current]:
                if current.get_type() == "Segment" and parents[current].get_type() == "Segment" :
                    ...
                    current.xd= (parents[current].xd - (1-parents[current].r)*parents[current].L*sp.sin(parents[current].th)*parents[current].thd) - current.r*current.L*sp.sin(current.th)*current.thd  
                    current.yd= (parents[current].yd + (1-parents[current].r)*parents[current].L*sp.cos(parents[current].th)*parents[current].thd) + current.r*current.L*sp.cos(current.th)*current.thd
